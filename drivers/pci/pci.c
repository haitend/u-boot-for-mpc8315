...
int __pci_skip_dev(struct pci_controller *hose, pci_dev_t dev)
{
	...
	if (dev == PCI_BDF(hose->first_busno, 0, 0)) {
		/* skip mpc8315 pcie ep*/
		unsigned int sub_vid_id;
		pci_hose_read_config_dword(hose, dev,
				PCI_SUBSYSTEM_VENDOR_ID, &sub_vid_id);
		if (sub_vid_id == ((PCI_DEVICE_ID_MPC8315E << 16)
...
#ifdef CONFIG_SKIP_PCI_SCAN
		if (pci_skip_dev(hose, dev) || (dev == PCI_BDF(0, 0 ,0)))
#else
