...
MPC8315ERDB_NAND_ECC_OFF_config \
MPC8315ERDB_PCISLAVE_config \
MPC8315ERDB_333MHz_config \
MPC8315ERDB_333MHz_NAND_config \
MPC8315ERDB_333MHz_ECC_OFF_NAND_config \
MPC8315ERDB_266MHz_config \
MPC8315ERDB_266MHz_NAND_config \
MPC8315ERDB_266MHz_NAND_ECC_OFF_config: unconfig
	@mkdir -p $(obj)include
	@mkdir -p $(obj)nand_spl
	@mkdir -p $(obj)board/freescale/mpc8315erdb
	@echo "" >$(obj)include/config.h ; \
	if [ "$(findstring _PCISLAVE_,$@)" ] ; then \
		echo "#define CONFIG_PCISLAVE"  >>$(obj)include/config.h ; \
		echo "... PCI Agent" ; \
	fi ; \
	if [ "$(findstring _333MHz_,$@)" ] ; then\
		echo -n "...333MHz core freq "; \
		echo "#define CONFIG_CORE_FREQ  CONFIG_SYS_FREQ_333MHz" >>$(obj)include/config.h ; \
	fi ; \
	if [ "$(findstring _266MHz_,$@)" ] ; then\
		echo -n "...266MHz core freq "; \
		echo "#define CONFIG_CORE_FREQ  CONFIG_SYS_FREQ_266MHz" >>$(obj)include/config.h ; \
	fi ; \
	if [ "$(findstring _NAND_,$@)" ] ; then \
		echo -n "... NAND " ; \
		echo "#define CONFIG_NAND_U_BOOT" >> $(obj)include/config.h ; \
	fi ; \
	if [ "$(findstring _ECC_OFF_,$@)" ] ; then \
		echo -n "... ECC_OFF " ; \
		echo "#define CONFIG_NAND_ECC_OFF" >> $(obj)include/config.h ; \
	fi ;
	@$(MKCONFIG) -a MPC8315ERDB ppc mpc83xx mpc8315erdb freescale ; \
	if [ "$(findstring _NAND_,$@)" ] ; then \
		echo "TEXT_BASE = 0x01000000" > $(obj)board/freescale/mpc8315erdb/config.tmp ; \
		echo "CONFIG_NAND_U_BOOT = y" >> $(obj)include/config.mk ; \
	fi ;
